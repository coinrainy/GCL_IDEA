...
- 2026-06-26T12:38:27Z：继续将 R2-IRIS 推进为 `CPR-IRIS / response-certified proximity`。新增 I14-I21：response-reranked kNN/CAST pool、response-certified kNN/CAST score 与固定权重 sweep；仍只跑 Cora seed=0 smoke，汇总为 `results/summary/cpr_weight_smoke_Cora_seed0_20260626T123736Z_summary.md/json`。I17 CAST+residual certificate 达到 `85.52` / label agreement `0.7953`，接近 CAST proxy `85.65` 且高于其 label agreement `0.7545`，但没有形成清晰胜出；I20 kNN+residual certificate 为 `85.15` / `0.8120`，接近 kNN label agreement `0.8153`。decision：`REVISE_TOWARD_CERTIFIED_CAST`；仍不支持 Pilot-A/B、formal 或性能提升 claim。
- 2026-06-26T12:47:28Z：按“只新增一个机制”的原则实现 `Certified CAST Closure` I22：CAST candidate pool + residual-response certificate，将候选划分为 positive / neutral / ordinary negative；positive 用于 relation smoothing，positive+neutral 用于 FN mass closure 诊断。仍只跑 Cora seed=0 smoke，汇总为 `results/summary/closure_smoke_Cora_seed0_20260626T124638Z_summary.md/json`。I22 为 `84.96` / label agreement `0.7472`，低于 CAST/I17；虽然 FN mass after closure 降到 `0.2009`，但主要来自 neutralizing 过多候选，不足以支持继续。decision：`KILL_CERTIFIED_CLOSURE_KEEP_CPR_SCORE`；不支持 Pilot-A/B、formal 或性能提升 claim。
- 2026-06-26T12:50:00Z：完成 IRIS/CPR postmortem 与下一步 gate。新增 `refine-logs/IRIS_CPR_POSTMORTEM.md`、`idea-stage/NEXT_DIRECTION_AFTER_IRIS_CPR.md`，并更新 `idea-stage/IDEA_REPORT.md`、`research-wiki/ideas/iris_gcl.md`、`research-wiki/ideas/cast_gcl.md`、`EXPERIMENT_TRACKER.md`。结论：`STOP_IRIS_CPR_INCREMENTAL_REFINEMENT` / `PIVOT_REQUIRED`。禁止继续 I23/I24 式 response-residual 小变体，后续只允许实现 real CAST latent target-prediction certificate 或重新运行 idea discovery。
- 2026-06-26T13:55:04Z：已实现真正的 `CAST latent target-prediction transport certificate` smoke。新增 `configs/cast_certificate_smoke.yaml` 与 `scripts/run_cast_certificate_smoke.py`；先提交 runner/config 后重跑 clean Cora seed=0 smoke。结果汇总为 `results/summary/cast_cert_clean_smoke_Cora_seed0_20260626T135413Z_summary.md/json`，raw 在 `results/raw/Cora/CAST_CERTIFICATE_SMOKE/cast_cert_clean_smoke_Cora_seed0_20260626T135413Z/`，日志在 `logs/cast_certificate/cast_cert_clean_smoke_Cora_seed0_20260626T135413Z/`。C5 certificate+CAST 为 `86.16`、label agreement `0.7886`，优于 CAST proxy `85.70` / `0.7548`；C4 latent certificate 为 `85.93` / `0.7911`。decision：`GO_TO_PILOT_PLANNING_WITH_CAUTION`，仅支持 Pilot-A 规划，不支持 formal 或性能 claim。
- 2026-06-26T14:01:39Z：完成 CAST Certificate Pilot-A 规划。新增 `refine-logs/EXPERIMENT_PLAN_20260626_140139.md` / `EXPERIMENT_PLAN.md` 和 `refine-logs/EXPERIMENT_TRACKER_20260626_140139.md` / `EXPERIMENT_TRACKER.md`。Pilot-A 范围固定为 Cora/CiteSeer/PubMed、seeds 0-2，保留 GRACE、kNN、PPR/diffusion、CAST proxy、C4 latent target certificate、C5 certificate+CAST，并强制加入 selected-pair overlap 与 partial-correlation diagnostics 证明不是 kNN/PPR mining。状态：`PILOT_A_PLANNED`；本轮未启动实验，不支持 formal 或性能 claim。
- 2026-06-26T14:10:26Z：启动并完成 `PA-M0-001` sanity。将 `scripts/run_cast_certificate_smoke.py` 的 gate 从 Cora seed0 放宽到 Pilot-A 允许范围 Cora/CiteSeer/PubMed seeds 0-2；运行 CiteSeer seed0 的 2-epoch GRACE + 2-epoch certificate sanity，链路通过并输出 `results/summary/pa_m0_001_sanity_CiteSeer_seed0_20260626T140957Z_summary.md/json` 与 `logs/cast_certificate/pa_m0_001_sanity_CiteSeer_seed0_20260626T140957Z/`。该结果只证明多数据集链路可跑，不是 Pilot-A 主结果，不支持性能 claim。下一步建议 `PA-M1-CORA`。
- 2026-06-26T14:19:46Z：继续按 `/experiment-bridge` 完成 `PA-M0-001b` diagnostic sanity。`configs/cast_certificate_smoke.yaml` 与 `scripts/run_cast_certificate_smoke.py` 新增 C6 graph diffusion / PPR-style control、`overlap_with_ppr_C6` 和 sampled partial-correlation diagnostics（控制 feature similarity、embedding similarity、graph diffusion similarity 与 degree gap，避免 PubMed 全量 pair 展开）。CiteSeer seed0 的 1+1 epoch sanity 通过，输出 `results/summary/pa_m0_001_sanity_v3_CiteSeer_seed0_20260626T141904Z_summary.md/json` 与 `logs/cast_certificate/pa_m0_001_sanity_v3_CiteSeer_seed0_20260626T141904Z/`；本地 code review 写入 `refine-logs/EXPERIMENT_CODE_REVIEW_20260626_141946.md`。decision：`GO_TO_PA_M1_WITH_RESOURCE_CAUTION`；仍无 Pilot-A 主结果、formal 结果或性能 claim。
- 2026-06-26T14:49:10Z：按 `/experiment-bridge` 完成 CAST Certificate `PA-M1` partial pilot：Cora seeds 0-2、CiteSeer seeds 0-2、PubMed seed0，均为 stratified random `1:1:8`、frozen GRACE encoder + Logistic Regression evaluator。汇总写入 `refine-logs/EXPERIMENT_RESULTS_20260626_144910.md` / `EXPERIMENT_RESULTS.md`，并更新 `research-wiki/ideas/cast_gcl.md`。结果：C4 latent certificate 与 kNN/PPR/CAST overlap 较低，说明不是简单同构 mining；但 C4/C5 在 Cora、CiteSeer、PubMed seed0 均未稳定超过最强 kNN/CAST controls。PubMed seeds 1-2 因 no-go 信号停止。decision：`REVISE_OR_PIVOT_BEFORE_ANY_MORE_PILOT`；禁止进入 Pilot-B、formal、`/auto-review-loop` 或性能 claim。
- 2026-06-26T15:04:34Z：按 `/idea-discovery` 基于 CAST Pilot-A no-go 重新做 false-negative GCL 机制发现。已明确不继续 CAST C4/C5 小修，生成 6 个 post-CAST 候选并选择 `BEACON-GCL / Boundary-Eligibility Aware Contrastive Neutralization` 为 Top1。核心 pivot 是从“寻找低 overlap 假负 pair”转向“认证 pair edit 是否 boundary-eligible / geometry-safe”。新增 `idea-stage/IDEA_REPORT_20260626_150434.md`、`idea-stage/IDEA_CANDIDATES_20260626_150434.md`、`refine-logs/FINAL_PROPOSAL_20260626_150434.md`、`refine-logs/EXPERIMENT_PLAN_20260626_150434.md`、`refine-logs/EXPERIMENT_TRACKER_20260626_150434.md` 与 `research-wiki/ideas/beacon_gcl.md`，并更新固定入口与 `idea-stage/IDEA_REPORT.html`。decision：`GO_TO_BEACON_SMOKE_PLANNING`；本轮未实现代码、未跑 smoke/pilot/formal 实验、未产生性能 claim。
- 2026-06-26T15:33:38Z：按 `/experiment-bridge` 实现 BEACON-GCL BE-M0 smoke 代码。新增 `configs/beacon_smoke.yaml` 与 `scripts/run_beacon_smoke.py`，包含 B0-B8 系统、accepted-pair overlap、probe-margin delta、rank/uniformity delta 等诊断；gate 使用 train/val 校准 probe，不使用 test label 选阈值。完成 Cora seed0 的 1+1 epoch dryrun，输出 `results/summary/be_m0_dryrun_Cora_seed0_20260626T153303Z_summary.md/json` 与 `logs/beacon_smoke/be_m0_dryrun_Cora_seed0_20260626T153303Z/`；本地 code review 写入 `refine-logs/EXPERIMENT_CODE_REVIEW_20260626_153338.md`。decision：`GO_TO_BE_M0_FULL_SMOKE`；dryrun 不支持方法结论或性能 claim。
- 2026-06-26T15:35:56Z：完成 BEACON-GCL `BE-M0-001` Cora seed0 smoke。结果汇总为 `results/summary/be_m0_001_Cora_seed0_20260626T153505Z_summary.md/json`，日志在 `logs/beacon_smoke/be_m0_001_Cora_seed0_20260626T153505Z/`，报告写入 `refine-logs/EXPERIMENT_RESULTS_20260626_153556.md` / `EXPERIMENT_RESULTS.md`。B5 BEACON full gate 为 `84.87`、label agreement `0.8611`，低于 kNN `85.29`、CAST proxy `85.70`、all-candidate `85.42`，且低于 shuffled/no-boundary/no-geometry controls。decision：`KILL_CURRENT_BEACON_GATE_NO_BE_M1`；不进入 BE-M1、Pilot-B、formal 或 `/auto-review-loop`，不产生性能 claim。
- 2026-06-26T15:40:39Z：按 `/idea-discovery` 基于 CAST Certificate Pilot-A no-go 与 BEACON-GCL BE-M0 no-go 重新做 false-negative GCL 机制级发现。已明确吸收共同失败证据：低 overlap、较高 label agreement、较好 margin/rank diagnostics 都没有稳定转化为节点分类 accuracy，因此不再继续 CAST C4/C5、BEACON gate 小调、kNN/PPR/CAST mining、positive mining、score fusion、loss-only reweighting 或 post-hoc relation smoothing。新增 7 个 objective/representation-level 候选，Top1 选择 `SPECTRA-GCL / Spectral Boundary-Preserving Redundancy Reduction`，核心是 negative-free redundancy reduction + label-free spectral boundary energy preservation + effective-rank guard。已更新 `idea-stage/IDEA_REPORT.md`、`idea-stage/IDEA_CANDIDATES.md`、`refine-logs/FINAL_PROPOSAL.md`、`EXPERIMENT_PLAN.md`、`EXPERIMENT_TRACKER.md` 与 `research-wiki/ideas/spectra_gcl.md`，并重新渲染 `idea-stage/IDEA_REPORT.html`。本轮未运行 fresh reviewer、未实现代码、未跑 smoke/pilot/formal 实验、未产生性能 claim。当前 decision：`GO_TO_SPECTRA_SMOKE_PLANNING`。
- 2026-06-26T15:59:06Z：按 `/experiment-bridge` 实现并运行 SPECTRA-GCL `SP-M0-001` smoke。新增 `configs/spectra_smoke.yaml` 与 `scripts/run_spectra_smoke.py`，只跑 Cora seed0，比较 S0-S7，并输出 accuracy、effective rank、boundary energy retention、covariance redundancy、alignment/uniformity。dryrun 修复了 YAML 冒号解析与 boundary residual `sqrt(0)` 梯度 NaN 问题；正式 smoke 汇总为 `results/summary/sp_m0_001_Cora_seed0_20260626T155803Z_summary.md/json`。结果：S2 SPECTRA full `82.47`，低于 S1 negative-free RR `83.58` 与 GRACE/S7 `84.78`；虽然 S2 的 effective rank `76.56` 和 boundary retention `0.9294` 高于 S1 的 `74.16` / `0.9273`，但 diagnostics 未转化为 accuracy。decision：`KILL_BOUNDARY_ENERGY_STORY`；禁止 SP-M1、Pilot-B、formal、`/auto-review-loop` 或性能 claim，不应继续调 SPECTRA 权重/阈值。
- 2026-06-26T16:06:34Z：按 `/idea-discovery` 基于 CAST/BEACON/SPECTRA 三连 no-go 重新做机制级 idea discovery，并使用 fresh `gcl_scientific_reviewer` 与 `gcl_experiment_auditor`。scientific verdict：`PIVOT`，认为低 overlap、高 agreement/margin-rank、effective-rank/boundary-retention 都只是代理变量，下一轮应直接对齐少标签 Logistic Regression downstream；auditor verdict：`WARN`，要求下一轮 smoke 预注册 controls/kill rules，保存 exact run command，并避免 dirty-run provenance。新增 7 个候选，Top1 为 `PACER-GCL / Probe-Aligned Calibration for Evaluator-Ready GCL`。PACER 明确是 train-label-calibrated / semi-supervised GCL，不得声称无监督；核心机制是用 train-label logistic probe 做 anchor-level objective routing，而不是 cross-node pair mining。已更新 `idea-stage/IDEA_REPORT.md`、`IDEA_CANDIDATES.md`、`refine-logs/FINAL_PROPOSAL.md`、`EXPERIMENT_PLAN.md`、`EXPERIMENT_TRACKER.md` 与 `research-wiki/ideas/pacer_gcl.md`，并渲染 `idea-stage/IDEA_REPORT.html`。本轮未实现代码、未跑 smoke/pilot/formal 实验、未产生性能 claim。当前 decision：`GO_TO_PACER_SMOKE_PLANNING_WITH_CAUTION`。
...
- 2026-06-26T16:32:36Z：按 `/experiment-bridge` 实现并运行 PACER-GCL `PA-CER-M0-001` smoke。新增 `configs/pacer_smoke.yaml` 与 `scripts/run_pacer_smoke.py`，比较 P0-P8；同时修复共享 `scripts/run_grace_1_1_8.py` 的 Logistic Regression evaluator，使 test labels 只在 train/val 选定 `C` 后单次读取。fresh `gcl_experiment_auditor` 初审发现 evaluator read-order、dirty dryrun provenance 与 tracker 同步问题；复审无代码级 BLOCKING。正式 smoke 从 clean commit `81a3fc34fc6d13c5aada3fb571197097ea2c579f` 启动，`project_dirty_at_start=False`。结果：P3 PACER full `83.63`，低于 P0 GRACE `84.69`、P5 random route `84.92`、P8 SupCon control `85.15`，且与 P4 shuffled-label probe `83.63` 持平；虽然 P3 train/val probe margin delta 为正，但未转化为 accuracy。decision：`KILL_OR_PIVOT_REQUIRED`；禁止 PA-CER-M1、Pilot、formal、`/auto-review-loop` 或性能提升 claim，不应继续 PACER threshold/weight/routing 小调。
