#!/usr/bin/env python
"""Run supervised GAT under the project's node-classification protocol."""
...
GAT_DIR = PROJECT_ROOT / "baselines" / "GAT"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="configs/gat_1_1_8.yaml")
    ...
    parser.add_argument("--output-heads", type=int, default=None)
...
def merged_dataset_config(config: dict[str, Any], dataset_name: str, args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for key in ("hidden_dim", "heads", "output_heads", "dropout", "add_self_loops", "negative_slope"):
        if key in overrides:
    ...
    if args.heads is not None:
        model_cfg["heads"] = args.heads
    if args.output_heads is not None:
        model_cfg["output_heads"] = args.output_heads
...
def train_one(
    ...
    result_dir = PROJECT_ROOT / args.result_root / dataset_name / "GAT"
    result_dir.mkdir(parents=True, exist_ok=True)
    existing = sorted(result_dir.glob(f"gat_1_1_8_{dataset_name}_seed{seed}_*.json"))
    ...
    model = GAT(
        ...
        output_heads=int(model_cfg["output_heads"]),
    ...
    result = {
        "run_id": f"gat_1_1_8_{dataset_name}_seed{seed}_{time.strftime('%Y%m%dT%H%M%SZ', time.gmtime())}",
        "method": "GAT",
        ...
        "baseline_commit_hash": git_rev(GAT_DIR),
        "baseline_dirty": git_dirty(GAT_DIR),
